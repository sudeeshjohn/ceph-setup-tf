...
for device in $(ls -1 $disk_path|egrep -v "[0-9]$"); do
    if [[ ! -b $device"1" ]]; then
        ...
        if [[ -z $storage_device && $device_size == ${volume_size} ]]; then
            ...
            echo "ENV{DEVTYPE}==\"disk\", ENV{SUBSYSTEM}==\"block\", ENV{DEVPATH}==\"$(sudo udevadm info --root --name="$storage_device" | sudo grep DEVPATH | sudo cut -f2 -d'=')\" SYMLINK+=\"$storage_disk_name\"" | sudo tee /lib/udev/rules.d/10-custom-ceph.rules;
